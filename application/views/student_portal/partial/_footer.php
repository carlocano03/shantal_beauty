...
<script>
    var temp_pass = "<?= $this->session->userdata('scholarIn')['temp_pass']?>";
    ...
    $(document).ready(function() {
        ...
        $(document).on('input', '#old_password', function() {
            ...
            $.ajax({
                url: "<?= base_url('portal/student_portal/main/check_old_pass')?>",
                method: "POST",
                data: {
                    old_pass: old_pass,
        ...
        if (form.checkValidity() === false) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "<?= base_url('portal/student_portal/main/update_password')?>",
                        method: "POST",
                        data: formData,
                        contentType: false,
                        processData: false,
                            ...
                            if (data.message == 'Success') {
                                ...
                                setTimeout(() => {
                                    window.location.href =
                                        "<?= base_url('main/logout/scholarIn');?>";
                                }, 2000);
                            } else {
                                Swal.fire({
                                    icon: 'warning',
